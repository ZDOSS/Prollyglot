...
Audio recovery uses one monotonic session clock and sequence across WASAPI
stream recreation. The first packet after reconnection is discontinuous so
resampling, buffering, and ASR input state reset without moving transcript time
backward or prematurely reaching Nemotron's utterance-duration boundary.
...
source has changed in its recognized text areas or undergone a broad scene
change, its text is discarded instead of being placed over a later scene.
Dimension changes invalidate old geometry immediately. Verified unchanged text
areas can refresh the source evidence after a slow pass; otherwise source age
travels with the OCR update and is checked again before and after translation.
The inexpensive gate aggregates complete tiles instead of isolated pixels and
allows a one-second periodic refresh so thin strokes, fades, or a missed first
pass cannot leave static text permanently undetected. Discarding an old result clears its text tracks, cancels pending
...
are checked for current source evidence when loading completes; disappeared
text is never sent to inference just because a cold model finally became ready.
Failed current regions retry at most twice after the first attempt, with 500 ms
and 1000 ms backoffs; a new text revision gets its own attempt budget. Audio-caption
...
absent, an already displayed translation remains readable for at most eight
seconds. Pending translations are removed, and late completion cannot introduce
a new label for disappeared text. If the same
...
instead receives the actual measured capture-space bounds of rendered labels
from the overlay. It filters matching raw OCR lines only when they overlap the
label, before nearby source lines are grouped. Current labels remain excluded;
...
The installed OCR source catalog supports Chinese, Japanese, English, and the
19 Latin-script languages present in the product catalog. Unsupported script
choices cannot start this OCR pack; existing unsupported preferences are shown
as unavailable until the user selects a valid source. Translation targets are
independent of OCR source coverage.

The integrated slice is accepted only if representative Chinese, Japanese, and Spanish
...
quality, latency, and resource use remain required evidence. The repeatable
Chinese/Spanish fixture, local stage evaluator, and observed full-display
small-text limitations are recorded in
[the visual reliability evaluation](docs/testing/VISUAL_RELIABILITY_EVALUATION.md).

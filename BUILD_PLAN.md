...
Long Japanese media then exposed a second translation-latency and layout failure. Translation waited behind both Nemotron's 20-second continuous-utterance boundary and older queued captions, while a provisional source row could span the bilingual grid and displace an older English result. Nemotron's pause-light safety boundary is now eight seconds, translation still begins only from finalized source text, and a bounded newest-first queue drops stale work before it can keep the live display permanently behind. The overlay distinguishes **English after pause…** from active **Translating…**, keeps every source/English pair in one stable row, and offers zero to three complete fading history rows. Privacy-safe timing diagnostics report model load, translation inference, queue wait, and stale-skip counts without recording caption text.
...
The first integration slices are present for owner evaluation: one explicitly downloaded Nemotron model can produce original-language English, Spanish, or Japanese captions and can run unconstrained automatic detection, while separate explicitly downloaded models translate finalized forced-Japanese or forced-Spanish text to English. Original, English, and bilingual output modes are wired through the overlay and transcript. Translation prioritizes the current live edge, and bilingual history retains complete paired rows rather than clipping or displacing one language. These remain experimental pre-release surfaces. Allowed-language constraints, automatic per-segment language reporting, broader test material, representative translation benchmarks, native Windows latency/resource evidence, and production quality gates are still pending.

...
- an explicit first-run model flow with atomic downloads, safe path validation, size/SHA-256 verification, background startup inspection, working removal, and persistent selection among four pinned local streaming models;
...
- optional pinned Japanese-to-English and Spanish-to-English translators that download only on request, verify every artifact, run locally on CPU/WebAssembly, and never delay the original caption path;
- Original, English, and Original + English output modes, with stable stacked or side-by-side source/translation pairs, independent colors, and zero to three fading prior caption rows;
...
The first real-Windows smokes confirmed selected-device capture and exposed startup-preview, overlay painting, Appearance dismissal, short-utterance, context-retention, and Settings-feedback defects. The latest owner re-smoke found better results and confirmed that Appearance, Transcript, and Settings now open and close correctly. Recognition of some short speech and accented dialogue remains inconsistent. Settings therefore offers Fast (43.1 MiB), Balanced (70.0 MiB), and Enhanced (181.4 MiB), plus an opt-in Nemotron 3.5 Streaming 0.6B model (650.6 MiB) for original-language English, Spanish, Japanese, or automatic detection. Nemotron is a CPU path in the current build, is not the default, and does not yet have enough representative evidence to promise better English or production-ready automatic detection. The language selection actively guides Nemotron, so a known language should be selected for best accuracy and Automatic is intended for mixed-language media.

Forced Japanese and Spanish now expose optional English translation. Their separate local q8 translators are 109.4 MiB and 113.8 MiB respectively; neither is bundled, downloaded, or loaded for English-only use. Real-model development-WebView checks passed for both directions, including verified install, offline cache reuse, Japanese model removal/reinstallation, transcript updates, and bilingual rendering. Native Windows media quality, cold-load latency, sustained resource use, and a broader translation sample still need owner evaluation before this becomes a production-quality claim. Automatic mixed-language recognition remains original-only until the ASR path reports a dependable detected language for each finalized segment. The remaining Milestone 2 gates include application capture, lifecycle and overlay validation, partial-caption latency measurement, and representative conversational model evidence. See [BUILD_PLAN.md](BUILD_PLAN.md) for milestone status and use the [Windows development smoke test](docs/testing/WINDOWS_SMOKE_TEST.md) for ordinary pre-release checks.

Translated output is enabled explicitly under **Caption output**; downloading a translator only makes that option available offline. Translation still waits for stable finalized source text: the overlay says **English after pause…** while recognition is provisional and **Translating…** only after translation actually begins. Nemotron now finalizes continuous pause-light speech at an eight-second safety boundary, and the bounded translator queue prioritizes the newest caption and skips stale backlog instead of remaining permanently behind. Source and English stay in one stable row in side-by-side mode, while **Appearance → Caption history** can retain zero to three complete fading prior rows without clipping them midway.

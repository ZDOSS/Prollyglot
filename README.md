...
Visual text translation now exists as an experimental, separately enabled Windows slice. **Screen translation** continuously watches one explicitly selected top-level window, display, or drawn live display region through `Windows.Graphics.Capture`; feeds transient frames through a latest-frame queue, change gate, PP-OCRv6 Small, and two-frame text stabilizer; and places a local translation near the original text that is already visible on screen. **Prominent text** is the default detection mode and suppresses low-confidence, very small, wrong-script, and common interface-noise results; **All detected text** remains available when small text matters. The optional OCR pack is a 30.4 MiB explicit download, and translation reuses the selected local language pack. Nothing is downloaded automatically. Audio and visual sessions are mutually exclusive for now. Application, display, and region selections share a camel-case-tested native contract, and the region selector is translucent. The control and overlay surfaces remain visible to ordinary screenshots; the capture loop instead filters the translations that Prollyglot currently draws, avoiding a feedback loop without making the app disappear from full-screen captures. The WebView workflow and actual OCR model initialization pass on the development host, but native Windows capture, DPI/multi-monitor positioning, performance, Stop responsiveness under real OCR load, and representative media quality still require owner testing.

The control app now opens as a full desktop workspace with persistent navigation for Captions, Screen translation, Transcript, Models, Appearance, and Settings. Captions and screen translation use desktop-width grouped panels instead of one long mobile-style form. A title-bar control switches to a focused compact utility that retains the current Start/Stop path and bottom navigation; the app remembers that choice and restores an appropriate window size for each mode.

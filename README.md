...
- Original, English, and Original + English output modes, with stacked or side-by-side bilingual captions and independent colors for the source and translation;
...
Translated output is enabled explicitly under **Caption output**; downloading a translator only makes that option available offline. When translated output is selected, Prollyglot now preloads the chosen local translator, shows a visible pending state, and keeps a finalized source caption on screen long enough for a delayed English result to replace it. The custom Windows title bar also has the explicit Tauri permissions required for dragging and its minimize, maximize, and close controls; the next native Windows run remains the final check for those operating-system interactions.

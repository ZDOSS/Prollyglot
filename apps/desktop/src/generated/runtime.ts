...
export type VisualCaptureSelection = { "kind": "portalWindow" } | { "kind": "portalDisplay" } | { "kind": "portalRegion" } | { "kind": "applicationWindow", sourceId: string, } | { "kind": "display", sourceId: string, } | { "kind": "region", displayId: string, region: PixelRect, };

export type VisualDetectionMode = "focused" | "allText";

export type VisualCaptureCapabilities = { windowsGraphicsCapture: boolean, portalScreenCast: boolean, systemPicker: boolean, desktopDuplicationExperiment: boolean, message?: string, };
...
export type VisualTextUpdate = { frameAgeMs: number, sessionId: SessionId, runtimeRevision: number, source: VisualCaptureGeometry, visible: Array<StableVisualTextRegion>, translationRequests: Array<StableVisualTextRegion>, removedTrackIds: Array<number>, };
...
export type VisualPresentationFrame = { sessionId: SessionId, runtimeRevision: number, presentationRevision: number, sourceWidth: number, sourceHeight: number, sourceLanguage: string, targetLanguage: string, scanning: boolean, anchored: boolean, regions: Array<VisualPresentationRegion>, };

export type VisualOverlayLabel = { trackId: number, textRevision: number, bounds: VisualRect, };

export type VisualOverlayLayout = { sessionId: SessionId, presentationRevision: number, labels: Array<VisualOverlayLabel>, };
